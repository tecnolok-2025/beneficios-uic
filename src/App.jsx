...
const emptyBenefit = {
  ...
  contactPhone: '3489 65 0104', contactEmail: 'uic@uic-campana.com.ar', companyContacts: [], agreementUrl: '', externalImageUrl: '', externalImageAlt: '', startDate: '', endDate: '', published: true
}
  ...
  return <header className="neo-header">
    <div className="neo-shell neo-nav">
      ...
      <nav className="neo-actions">
        <span className="neo-version"><small>NUEVO PORTAL</small><strong>v{health?.version || '3.2.1'}</strong></span>
...
function Editor({ items, selected, setSelected, reload }) {
  ...
  return <div className="neo-admin-layout"><aside className="neo-admin-list"><label className="neo-admin-search"><Search/><input value={listQuery} onChange={event => setListQuery(event.target.value)} placeholder="Buscar beneficio o empresa" aria-label="Buscar beneficio o empresa" autoComplete="off"/>{listQuery && <button type="button" onClick={() => setListQuery('')} aria-label="Limpiar búsqueda"><X/></button>}</label><div className="neo-admin-search-meta"><span>{listQuery ? `${filteredItems.length} de ${items.length} resultados` : `${items.length} beneficios`}</span></div><button className="neo-new-benefit" onClick={() => setSelected(null)}><Plus/> Nuevo beneficio</button>{filteredItems.length ? filteredItems.map(item => <button className={selected?.id === item.id ? 'active' : ''} key={item.id} onClick={() => setSelected(item)}><strong>{item.title}</strong><span>{item.partner || 'Sin empresa informada'}</span></button>) : <div className="neo-admin-no-results"><Search/><strong>Sin coincidencias</strong><span>Probá por beneficio, empresa, rubro o palabra clave.</span></div>}</aside>
    <section className="neo-editor"><div className="neo-editor-title"><div><span>{selected ? 'EDITAR BENEFICIO' : 'NUEVO BENEFICIO'}</span><h2>{selected?.title || 'Crear una oportunidad'}</h2></div></div><div className="neo-form-grid">
      ...
      <div className="neo-full neo-uic-contact-heading"><span>CONTACTO UIC</span><p>Este contacto institucional se mantiene separado de los contactos de la empresa.</p></div><Field label="Persona de contacto UIC" value={form.contactName} onChange={value => change('contactName', value)}/><Field label="Teléfono UIC" value={form.contactPhone} onChange={value => change('contactPhone', value)}/><Field label="Correo UIC" type="email" value={form.contactEmail} onChange={value => change('contactEmail', value)}/><Field label="Alcance" value={form.scope} onChange={value => change('scope', value)}/><CompanyContactsEditor contacts={form.companyContacts || []} onChange={value => change('companyContacts', value)}/><div className="neo-full"><Field label="Link de información / convenio" value={form.agreementUrl} onChange={value => change('agreementUrl', value)}/></div><div className="neo-full"><Field label="Imagen externa / escudo (URL)" value={form.externalImageUrl || ''} onChange={value => change('externalImageUrl', value)}/><Field label="Texto alternativo de imagen" value={form.externalImageAlt || ''} onChange={value => change('externalImageAlt', value)}/></div><label className="neo-check"><input type="checkbox" checked={form.published !== false} onChange={event => change('published', event.target.checked)}/> Publicar en el portal</label><label className="neo-upload"><Upload/> <span>{files.length ? `${files.length} imagen${files.length === 1 ? '' : 'es'} seleccionada${files.length === 1 ? '' : 's'}` : 'Agregar flyers'}</span><input type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif" onChange={event => { setError(''); setFiles([...event.target.files]) }}/></label>{files.length > 0 && <div className="neo-upload-files">{files.map(file => <span key={`${file.name}-${file.size}`}>{file.name} · {(file.size / 1024).toFixed(0)} KB</span>)}</div>}
