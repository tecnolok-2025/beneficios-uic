  ...
  return <header className="neo-header">
    <div className="neo-shell neo-nav">
      ...
      <nav className="neo-actions">
        <span className="neo-version"><small>NUEVO PORTAL</small><strong>v{health?.version || '3.6.0'}</strong></span>
...
function BenefitCard({ item }) {
  ...
  const needsValidation = item.status === 'revalidacion'
  return <article className={`neo-card ${needsValidation ? 'neo-card-validation' : ''}`} onClick={() => go(`/beneficio/${item.slug}`)} onKeyDown={event => { if (event.key === 'Enter') go(`/beneficio/${item.slug}`) }} tabIndex="0">
    <div className="neo-card-head"><span className="neo-card-icon"><Building2/></span><span className="neo-card-category">{item.category}</span><div className="neo-card-badges">{isNew && <span className="neo-new"><Sparkles/> NUEVO</span>}{needsValidation && <span className="neo-validation"><CircleAlert/> A CHEQUEAR VALIDACIÓN</span>}</div></div>
...
function Home() {
  ...
  const showLatestAgreement = () => go('/beneficio/uipba-open-english-business-80-off')
  useEffect(() => {
    ...
    <section className="neo-hero">
      ...
      <div className="neo-shell neo-hero-layout">
        ...
        <aside className="neo-impact">
          ...
          <div className="neo-big-number"><strong>{items.length || 45}</strong><span>beneficios<br/>en el portal</span></div>
          <div className="neo-impact-grid"><div><strong>{categories.length ? categories.length - 1 : 14}</strong><span>rubros</span></div><div><strong>{items.filter(item => item.status === 'activo').length || 37}</strong><span>beneficios activos</span></div><div><strong>{items.filter(item => item.status === 'revalidacion').length || 8}</strong><span>a validar</span></div></div>
...
function Detail({ slug }) {
  ...
  return <><Header/><main className="neo-detail">
    <section className="neo-detail-hero"><div className="neo-shell"><button onClick={() => go('/')}><ArrowLeft/> Volver al catálogo</button><div className="neo-detail-tags"><span className="neo-detail-category">{item.category}</span>{item.status === 'revalidacion' && <span className="neo-validation"><CircleAlert/> A CHEQUEAR VALIDACIÓN</span>}</div><h1>{item.title}</h1><p><Building2/> {item.partner}</p></div></section>
    <div className="neo-shell neo-detail-layout"><div className="neo-detail-body">
      {item.status === 'revalidacion' && <section className="neo-validation-panel"><span><CircleAlert/> ESTADO DEL BENEFICIO</span><h2>A chequear validación con UIPBA</h2><p>Existe un antecedente institucional publicado, pero UIC debe confirmar vigencia y condiciones actuales antes de gestionarlo como beneficio activo.</p></section>}
    ...
    </div><aside className="neo-contact"><span>CONTACTO UIC</span><h2>{item.status === 'revalidacion' ? 'Consultá la validación' : 'Solicitá este beneficio'}</h2><p>{item.status === 'revalidacion' ? 'La UIC verificará con UIPBA si el beneficio continúa vigente y cuáles son sus condiciones actuales.' : 'La UIC verificará tu condición de socio y te orientará para acceder.'}</p>{item.contactName && <div><Users/>{item.contactName}</div>}{item.contactPhone && <a onClick={contact} href={`tel:${item.contactPhone}`}><Phone/>{item.contactPhone}</a>}{item.contactEmail && <a onClick={contact} href={`mailto:${item.contactEmail}`}><Mail/>{item.contactEmail}</a>}</aside></div>
